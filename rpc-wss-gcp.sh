...
function configure_node() {
  ...
  echo "testing outside" >> /var/log/test.log
  if [ "${option}" == "wss" ]; then
  echo "testing inside wss loop" >> /var/log/test.log
    if ! grep -q bootnodes "${BOR_START_SH}"; then
        echo "testing inside bootnodes" >> /var/log/test.log
    ...
    sed -i 's/--http.port.*/--wss.port '${port}' \\/g' "${BOR_START_SH}"
  ...
  echo "testing inside wss loop" >> /var/log/test.log
    ...
    sed -i 's/--http.port.*/--http.port '${port}' \\/g' "${BOR_START_SH}"
  else
    echo "Incorrect Port option specified! Please specify wss or rpc only..."
  fi
}
